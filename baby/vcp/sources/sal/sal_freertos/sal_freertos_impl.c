        ...
        *puiId = search_id;
        ...
        if (status == pdTRUE)
        {
            ...
            if (search_id < (uint32)SAL_MAX_QUEUE)// CS : Redundant Condition
            {
                ...
                {
                    (void)FR_MemCopy(&pValidBuffer, (const void *)&pQueueBuffer, sizeof(uint8 *));
        ...
        if (uxItemSize != 0UL) //CS : Uninitialized Variable
        {
            ...
            if (pushCnt != 0UL)
            {
                for (i =0; i < pushCnt; i++)
                {
                    ...
                    (void)FR_MemCopy(&queueHandle, (const void *)&address, sizeof(QueueHandle_t));
                    (void)FR_MemCopy(&queueDataPtr, &pData, sizeof(uint8 *));
...
static SALRetCode_t FR_TaskSleep (uint32 uiMilliSec)
{
    ...
    {
        ticks = (uint32)((uiMilliSec * configTICK_RATE_HZ) / 1000UL);
        vTaskDelay(ticks);
    }
