...
#define TEST                      0
#define WPS_FIFO_CAP             (NUM_WAYPOINTS)

// IMU 데이터 저장용 전역 변수
//static IMU_Data_t imu_data;

typedef struct {
    PP_Waypoint buf[WPS_FIFO_CAP];
    uint8_t head;   // next write
    uint8_t tail;   // next read (우린 pop 안 하고 snapshot만 할 거라 tail 고정해도 됨)
    uint8_t count;
} wps_fifo_t;

static wps_fifo_t g_wps_fifo;
...
/*===========ring buf function ==================*/
static void wps_fifo_init(wps_fifo_t* q)
{
    q->head = 0;
    q->tail = 0;
    q->count = 0;
...
static void wps_fifo_push_overwrite(wps_fifo_t* q, PP_Waypoint wp)
{
    if (q->count == WPS_FIFO_CAP) {
        // overwrite oldest -> tail 한 칸 전진
        q->tail = (uint8_t)((q->tail + 1) % WPS_FIFO_CAP);
        q->count--;
    }

    q->buf[q->head] = wp;
    q->head = (uint8_t)((q->head + 1) % WPS_FIFO_CAP);
    q->count++;
}

static uint8_t wps_fifo_snapshot(const wps_fifo_t* q, PP_Waypoint* out, uint8_t out_cap)
{
    uint8_t n = q->count;
    ...
    for (uint8_t i = 0; i < n; i++) {
        out[i] = q->buf[idx];
        idx = (uint8_t)((idx + 1) % WPS_FIFO_CAP);
    }
    return n;
}
...
int update_follower_steer(to_vcp_msg_t* msg)
{
    ...
    // pose 가져오고 (※ 테스트용 0 덮는 거는 지금은 유지)
    uint8_t ret = (uint8_t)Pose_Get(&pose);

    if (ret != 0) {
        mcu_printf("[follow_steer] pose get err=%d\n", (int)ret);
        return 1;
    }

    // =========================
    // Waypoint FIFO에 현재 leader 좌표 push (mm -> m, float)
    // =========================
    if (msg->wp_valid) {
        PP_Waypoint wp;
        wp.x = ((float)msg->leader_x_mm) * 0.001f;   // mm -> m
        wp.y = ((float)msg->leader_y_mm) * 0.001f;   // mm -> m
        wps_fifo_push_overwrite(&g_wps_fifo, wp);
    }

    if (msg->mode == MODE_FOLLOW_WAYPOINT) {

        // FIFO -> wps[] 배열로 snapshot
        uint8_t n = wps_fifo_snapshot(&g_wps_fifo, wps, NUM_WAYPOINTS);

        // n이 0이면 PP 못 돌림 (전방점 없음으로 5 뜰 확률 99%)
        if (n == 0) {
            mcu_printf("[follow_steer] no wp in fifo\n");
            steer_angle_rad = 0.0f;
            return 2;
        }

        // PP 계산
        ret = (uint8_t)pp_compute_steer(&pp_handler, &pose, wps, (size_t)n, &steer_angle_rad);
        if (ret != 0) {
            mcu_printf("[follow_steer] pp err=%d\n", (int)ret);
            return 2;
        }
    }
    else if (msg->mode == MODE_FOLLOW_VISION) {
        (void)steer_from_aruco_q15(msg->aruco_x_norm_q15, msg->aruco_dist_mm, &steer_angle_rad);
    }

    // 디버그(printf 제약 때문에 정수로)
    mcu_printf("[follow_steer] yaw=%d steer_deg_x1000=%d\n",
               (int)(pose.yaw * 1000),
               (int)(RAD2DEG(steer_angle_rad) * 1000));

    mcu_printf("[follow_steer] last_idx=%d x_v=%d y_v=%d ld2=%d\n",
               (int)pp_handler.last_target_idx,
               (int)(pp_handler.last_target_x_d * 1000),
               (int)(pp_handler.last_target_y_d * 1000),
               (int)(pp_handler.last_ld2 * 1000));
