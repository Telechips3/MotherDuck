    ...
    uint32 uiTxBufSize)
{
    ...
    for (i = 0; i < cnt; i++)
    {
        ...
        if (tx != (uint32 *)NULL_PTR)
        {
            ...
            {
                write_data = (((uint32)tx[3] << 24UL) | ((uint32)tx[2] << 16UL) | ((uint32)tx[1] << 8UL) | ((uint32)tx[0] << 0));
            }
        ...
        else
        {
            ...
            if ((dmaicr & BSP_BIT_29) != 0UL)
            {
                ...
                if (gpsb[uiCh].dAsyncRxBuf != NULL_PTR)
                {
                    ...
                    (void)SAL_MemCopy((void *)(gpsb[uiCh].dTxDma.dbAddr), gpsb[uiCh].dAsyncTxBuf, copy_length * 4);
                }
                ...
                {
                    /* Notify the completion of transfer */
                    // if ((gpsb[uiCh].dAsyncTxBuf != NULL_PTR) && (gpsb[uiCh].dAsyncRxBuf == NULL_PTR))
                    if ((gpsb[uiCh].dAsyncTxBuf != NULL_PTR))
                    {
                        event = GPSB_EVENT_TXCOMPLETE;
                    }
                    else
                    {
                        event = GPSB_EVENT_COMPLETE;
    ...
    uint32 uiSize)
{
    ...
    if (uiSize > 256)
    {
        mcu_printf("Invaild data size for TX FIFO\n");
    }
    ...
    {
        mcu_printf("ch %d bpw value [%d] is invaild\n", uiCh, bpw);
    }
}
...
void GPSB_ClearDMABuffer(uint8 uiCh, uint32 size)
{
    SAL_MemSet(gpsb[uiCh].dTxDma.dbAddr, 0x0, size);
