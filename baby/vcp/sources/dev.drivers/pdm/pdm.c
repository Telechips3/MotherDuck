...
/*
***************************************************************************************************
*                                           PDM_SetOperationMode
*
* Set Operation mode of PDM
...
***************************************************************************************************
*/
static SALRetCode_t PDM_SetOperationMode
(
    uint32                              uiChannel,
    uint32                              uiOperationMode
)
{
    uint32          uiVal           = 0;
    uint32          uiModuleId      = 0;
    ...
    if(uiChannel < PDM_OUT_CH_MAX)
    {
        ...
        if((uiModuleId < PDM_TOTAL_MODULES) && (uiChannelId < PDM_TOTAL_CH_PER_MODULE))
        {
            PDMHandler[uiChannel].chModeCfgInfo.mcOperationMode = uiOperationMode;
            ...
            uiVal = (uiVal & ~((uint32)0xFU << PDM_GetOPModeOperationModeReg(uiChannelId))) |
                  ((uint32)uiOperationMode << PDM_GetOPModeOperationModeReg(uiChannelId));
...
)
{
    ...
    ret |= (uint32)PDM_SetOperationMode(uiChannel, pModeConfig->mcOperationMode);
...
)
{
    ...
    else if(PDM_ON == PDM_GetChannelStatus(uiChannel))
    {
