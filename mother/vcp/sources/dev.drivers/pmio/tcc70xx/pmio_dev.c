...
(
    ...
    static uint32   gTimerForDbg        = 0UL;
#endif
...
static uint32   gModeForDbg          = 0UL;
static uint32   gReasonForDbg          = 0UL;
...
{
    uint32 uiReverseSrcOffset = 0UL;
    uint32 uiDepthRegAdr = 0UL;
    uint32 uiDepthRegVa = 0UL;
...
)
{
    ...
    gReasonForDbg      = (uint32)tReason;
    gModeForDbg      = (uint32)tMode;
...
)
{
    ...
    {
        (void)GPIO_Config(PMIO_VA_BU_DET_GPK, GPIO_FUNC((uint32)0)|GPIO_INPUT|GPIO_INPUTBUF_EN);
    }

    if( PMIO_VA_ACC_DET_GPK > GPIO_GPK(0))
    {
        (void)GPIO_Config(PMIO_VA_ACC_DET_GPK, GPIO_FUNC((uint32)0)|GPIO_INPUT|GPIO_INPUTBUF_EN);
        ...
        case PMIO_MODE_POWER_ON:
            {
                ...
                PMIO_REG_PMGPIO_CTL |= ( PMIO_VA_BIT21|PMIO_VA_BIT20);
                PMIO_REG_PMGPIO_CTL &= (~PMIO_VA_BIT16);
        ...
        case PMIO_MODE_POWER_DOWN:
            {
                ...
                //retention on, init off, power on
                PMIO_REG_PMGPIO_CTL &= (~(PMIO_VA_BIT21|PMIO_VA_BIT20|PMIO_VA_BIT16));
...
)
{
    ...
    else if((PMIO_REG_PMGPIO_STR & (uint32)PMIO_VA_BU_DET_PORT) == 0UL)
    {
...
void PMIO_SetGpk(uint32 uiPort32)
{
    uint32 uiComplexBit;
            ...
            if( ((puiNotiIrqSrcDirect>>uiDiIdx) & 0x1UL) == 0x1UL)
            {
                ...
                if(gtPMIONoti.uiTaskId == 0)
                {
                                    ...
                                    NULL)) == SAL_RET_SUCCESS)
                    {
                        ...
                        if ((SAL_EventCreate((uint32 *)&(gtPMIONoti.uiEventId), \
